...
ofxFFmpegRecorder::ofxFFmpegRecorder()
    : m_FFmpegPath("ffmpeg"), m_OutputPath(""), m_IsRecordVideo(false), m_IsRecordAudio(false), m_IsOverWrite(false), m_IsPaused(false), m_VideoSize(0, 0), m_BitRate(2000), m_AddedVideoFrames(0), m_Fps(30.f), m_CaptureDuration(0.f), m_TotalPauseDuration(0.f), m_DefaultVideoDevice(), m_DefaultAudioDevice(), m_VideCodec("mpeg4"), m_CustomRecordingFile(nullptr), m_DefaultRecordingFile(nullptr)
{
}
...
//-------------------------------------------
size_t ofxFFmpegRecorder::addSingleFrame(const ofPixels &pixels)
{
    if (m_IsPaused)
    {
        LOG_NOTICE("Recording is paused.");
